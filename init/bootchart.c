...
//+[TCCQB] QuickBoot BootChart
	...
	int pval = 0;
	char prop_allow[PROP_VALUE_MAX];	// BC - To get boot mode property.
	memset(prop_allow, 0, PROP_VALUE_MAX);

	pval = property_get(QBMANAGER_BOOT_WITH, prop_allow);
	INFO("bootchart_init : %s : %s\n", QBMANAGER_BOOT_WITH, prop_allow);
	
	if ( bootchart_QB == 0 && !strcmp(prop_allow, "true"))
	{		
	...
	INFO("bootchart_init : Start bootchart init!!\n");
