...
static void handle_generic_device_event(struct uevent *uevent)
{
    ...
    if (!strncmp(uevent->subsystem, "usb", 3)) {
         if (!strcmp(uevent->subsystem, "usb")) {
            // 2017-11-07 Add Logs for USB add & remove by yongki.kim@mobis
            if (!strcmp(uevent->action, "add") || !strcmp(uevent->action, "remove")) {
                ERROR("handle usb event. devname:%s, action:%s\n",
                uevent->device_name, uevent->action);
            }
			// Debug Log END
