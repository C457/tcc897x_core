...
static void runtime_format(char *blk_device)
{
    pid_t pid;
    int status;
    int ret;

    INFO("Running %s on %s\n", MAKE_EXT4FS_BIN, blk_device);
    pid = fork();
    if (pid > 0) {
        ...
        /* child, run checker */
        execlp(MAKE_EXT4FS_BIN, MAKE_EXT4FS_BIN,blk_device, (char *)NULL);
