...
/* readdir2 porting -> start */
static int handle_readdir_avn(struct fuse* fuse, struct fuse_handler* handler,
        const struct fuse_in_header* hdr, const struct fuse_read_in* req)
{
    char buffer[8192] = {0, };
    struct fuse_dirent_avn *fde = (struct fuse_dirent_avn*) buffer;
    struct dirent_avn *de;
    struct dirhandle_avn *h = id_to_ptr(req->fh);
    ...
    fde->type = de->d_type;
    fde->namelen = strlen(de->d_name);
    memcpy(fde->name, de->d_name, (__u32)de->d_reclen);

    /* add for readdir_avn */
    fde->mode = de->d_mode;
    fde->mtime = de->d_mtime;
    fde->size = de->d_filesize;

    fuse_reply(fuse, hdr->unique, fde,
            FUSE_DIRENT_ALIGN(sizeof(struct fuse_dirent_avn) + fde->namelen));
    return NO_STATUS;
}

static int handle_readdirplus_avn(struct fuse* fuse, struct fuse_handler* handler,
        const struct fuse_in_header* hdr, const struct fuse_read_in* req)
{
    char buffer[8192] = {0, };
    struct fuse_dirent_avn *fde = (struct fuse_dirent_avn*) buffer;
...
/* readdir2 porting -> start */
    case FUSE_READDIR_AVN: {
        const struct fuse_read_in *req = data;
        return handle_readdir_avn(fuse, handler, hdr, req);
    }

    case FUSE_READDIRPLUS_AVN: {
        const struct fuse_read_in *req = data;
        return handle_readdirplus_avn(fuse, handler, hdr, req);
    }
/* readdir2 porting -> end */
